...
public class TrainService {
    ...
    public int addTrain(AddTrainEntryDto trainEntryDto){
        Train train = new Train();

        List<Station> listofStation = trainEntryDto.getStationRoute();
        String route ="";
        for (int i=0;i<listofStation.size();i++){
            if(i==listofStation.size()-1)
                route += listofStation.get(i);
            else
                route += listofStation.get(i) + ",";
        }
